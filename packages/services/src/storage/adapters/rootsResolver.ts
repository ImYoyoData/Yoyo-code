/**
 * 数据根解析：R1 = 家目录下的 .yoyo-code（永远存在），R2 = 自定义数据存储路径下的 .yoyo-code（仅当设置了且 ≠ 家目录）。
...
import { YOYO_CODE_DATA_DIR_NAME } from "../../paths.js";
...
export function resolveStorageRoots(params: {
  ...
  const roots: StorageRootSpec[] = [
    { id: "home", path: join(home, YOYO_CODE_DATA_DIR_NAME), hasCustomDataBaseDir },
  ];
  if (hasCustomDataBaseDir) {
    roots.push({
      id: "dataBaseDir",
      path: join(dataBase, YOYO_CODE_DATA_DIR_NAME),
