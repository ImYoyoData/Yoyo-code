...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "zcode") {
    return workspacePath ? join(baseDir, ".yoyo-code") : join(baseDir, ".yoyo-code", "cli");
  }
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".yoyo-code", "config.json");
