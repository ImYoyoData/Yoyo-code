...
function ensurePluginManagementWorkspacePath(): string {
  const workspacePath = join(getDataBaseDir(), ".yoyo-code", PLUGIN_MANAGEMENT_WORKSPACE_DIR_NAME);
...
// CLI/bootstrap MCP servers configured in ~/.yoyo-code/cli/config.json (mcp.servers), which the agent
...
export function createZCodeAgentService(
  ...
  // homedir() 即用户家目录，全局根 `~/.yoyo-code/workflows/` 因此解析到真实目录。
