...
function resolveDeployedZCodeAgentBinaryCommand(
  ...
  // SSH 远端把 zcode-server.cjs 单文件部署到 ~/.yoyo-code/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 zcode-agent 已经部署到 ~/.yoyo-code/server/agents/glm/，
  ...
  // packagedResourcesPath、~/.yoyo-code/server/agents/glm、bundled-agents 等），
