...
export const ZCODE_PROTOCOL_NAME = "Yoyo Code Protocol" as const;
...
// `<cwd>/.yoyo-code/workflows/`（挂载时快照会漏掉手改的文件）。形状与 @zcode/contracts 的
...
// 作用域两档：项目档落 `<cwd>/.yoyo-code/workflows/`、全局档落 agent 机器的 `~/.yoyo-code/workflows/`。作用域由文件所在目录推得，frontmatter 不存 scope。
...
export const zcodeWorkflowsListParamsSchema = z
  .object({
    workspace: zcodeWorkspaceRefSchema,
    // 缺省即 `project`（本项目档）。给 `global` 时改扫本机 `~/.yoyo-code/workflows/`；此时 `workspace`
