...
!ifndef ZCODE_INSTALLER_DEFAULT_LOG_PATH
  !define ZCODE_INSTALLER_DEFAULT_LOG_PATH "$TEMP\yoyo-code-installer.log"
!endif
!ifndef ZCODE_INSTALLER_ELEVATED_LOG_PATH
  !define ZCODE_INSTALLER_ELEVATED_LOG_PATH "$WINDIR\Logs\yoyo-code-installer.log"
!endif
...
!ifndef ZCODE_INSTALL_MANIFEST_NAME
  !define ZCODE_INSTALL_MANIFEST_NAME ".yoyo-code-install-manifest"
!endif

!ifndef ZCODE_UNINSTALLER_LOG_PATH
  !define ZCODE_UNINSTALLER_LOG_PATH "$TEMP\yoyo-code-uninstaller.log"
!endif
...
!ifndef BUILD_UNINSTALLER
  ...
  Function ZCodeShowUninstallerCleanupDetails
    ...
    zcodeShowUninstallerDetailsRead:
      ...
      SetDetailsPrint listonly
      DetailPrint "Yoyo Code: cleanup-log $R1"
  ...
  !macro customUnInstallCheck
    ...
    ${if} $R0 != 0
      ...
      SetDetailsPrint listonly
      DetailPrint "Yoyo Code: cleanup-failed exit-code=$R0"
      Call ZCodeShowUninstallerCleanupDetails
...
!macro customPageAfterChangeDir
  ...
  Function ZCodeFindNestedDataDir
    ...
    IfFileExists "$R9\.yoyo-code\*.*" 0 +2
      StrCpy $R2 "$R9\.yoyo-code"
    StrCmp $R2 "" 0 zcodeFindNestedDataDirDone
    IfFileExists "$R9\.yoyo-code" 0 zcodeFindNestedDataDirListChildren
      StrCpy $R2 "$R9\.yoyo-code"
    StrCmp $R2 "" 0 zcodeFindNestedDataDirDone
  ...
  Function ZCodeBlockInstallDirContainsData
    ...
    ;  用户可能把数据存储目录放进安装目录，Windows 更新覆盖安装目录时会清掉 .yoyo-code。
    ...
    zcodeInstallDirDataBlockCheckDir:
      ; 旧阻断只检查最终安装目录直属的 .yoyo-code，漏掉 data\.yoyo-code 等子目录数据。
      ; 安装器覆盖安装时会管理整个安装目录树，递归命中任意 .yoyo-code 都必须阻断。
      Push "$R0"
      Call ZCodeFindNestedDataDir
    ...
    zcodeInstallDirDataBlockFound:
      ...
      !insertmacro MUI_HEADER_TEXT "需要修改安装目录" "当前安装目录或其子目录包含 Yoyo Code 数据目录"
      nsDialogs::Create 1018
      Pop $0
      StrCmp $0 error zcodeInstallDirDataBlockDialogFailed 0

      ${NSD_CreateLabel} 0u 0u 300u 44u "检测到该安装目录或其子目录中存在 .yoyo-code 数据目录：$\r$\n$R2"
      Pop $1
    ...
    zcodeInstallDirDataBlockDialogFailed:
      MessageBox MB_OK|MB_ICONSTOP "检测到安装目录或其子目录中存在 .yoyo-code 数据目录，安装已停止。请重新运行安装器并选择其他安装目录。"
      SetErrorLevel 1
      Quit
