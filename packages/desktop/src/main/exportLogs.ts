...
function getZCodeCliDir() {
  return join(homedir(), ".yoyo-code", "cli");
}
...
function getCuaHelperRunDir() {
  return join(homedir(), ".yoyo-code", "computer-use", "run");
}
...
function isExcludedRelativePath(relativePath: string): boolean {
  ...
  // 过去显式收集 ~/.yoyo-code/cli/debug 会把这类上下文带进手动导出和反馈完整日志，这里按目录段统一跳过。
  ...
  // ~/.yoyo-code/v2/dev 保存 stdio-traffic 等高频协议流，真实机器上会累计到 GB 级。
...
function shouldApplyLogExportRetention(archivePath: string): boolean {
  ...
  if (
    ...
    normalizedArchivePath.startsWith(".yoyo-code/cli/log/")
  ) {
    return true;
  }
...
async function createLogArchiveArtifacts(
  ...
  // GLM / zcode-cli 的运行日志写在 ~/.yoyo-code/cli/log，不在应用主数据目录 ~/.yoyo-code/v2 下。
  // 如果导出日志只扫描 v2，定位 agent CLI 启动、协议或崩溃问题时会缺少最关键的原生侧日志。
  await collectLogArchiveFilesFromDirectory(
    zcodeCliLogDir,
    posix.join(".yoyo-code", "cli", "log"),
    visitedDirs,
    files,
  );
  ...
  // 二者都不在 ~/.yoyo-code/cli/log 下，需要额外收集才能完整还原现场。
  await collectLogArchiveFile(
    join(zcodeCliDir, "config.json"),
    posix.join(".yoyo-code", "cli", "config.json"),
    files,
  );
  await collectLogArchiveFilesFromDirectory(
    join(zcodeCliDir, "rollout"),
    posix.join(".yoyo-code", "cli", "rollout"),
    visitedDirs,
    files,
  );
  ...
  // ~/.yoyo-code/computer-use/run/<socket>.exit.log，既不在 app data 也不在 ~/.yoyo-code/cli 下。
  // 同目录下有 .tokens broker 凭据，因此按文件名白名单只收 *.exit.log，不递归该目录。
  await collectLogArchiveFilesByName(
    getCuaHelperRunDir(),
    posix.join(".yoyo-code", "computer-use", "run"),
...
export async function createFeedbackLogArchiveFromExportLogs(
  ...
  return createFeedbackDiagnosticArchive({
    sources: [
      ...
      { directory: getZCodeCliLogDir(), archivePrefix: ".yoyo-code/cli/log" },
      {
        directory: getCuaHelperRunDir(),
        archivePrefix: ".yoyo-code/computer-use/run",
        exitLogsOnly: true,
      },
    ],
