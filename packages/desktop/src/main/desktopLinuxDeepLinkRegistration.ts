...
const LINUX_DEEP_LINK_DESKTOP_FILE = "yoyo-code.desktop";
const LINUX_DEEP_LINK_MIME_TYPE = "x-scheme-handler/yoyo-code";
// 归属标记：用于识别用户级 desktop 文件是否由本应用写入。
// 这里刻意保留上游的 Comment 文本：它是历史版本写过的同一行，改了就无法再识别并清理
// 旧版本遗留的用户级条目。它只用于归属判定，不是展示文案。
...
function resolveLinuxDeepLinkCommand(params: {
  ...
  return {
    ...
    // AppImage 的 yoyo-code:// 回调会由 xdg-open 按 .desktop Exec 二次启动。
...
function createLinuxDeepLinkDesktopEntry(params: {
  ...
  const productName = params.productName ?? "Yoyo Code";
  const iconName = params.iconName ?? "yoyo-code";
  const command = {
...
function removeOwnedUserDesktopEntry(
  ...
  if (!isOwnedDesktopEntry(desktopFilePath)) {
    logger.warn("[deep-link] Linux 用户级 yoyo-code.desktop 非本应用写入，保留不清理", {
      desktopFilePath,
    });
    return;
  }
  try {
    ...
    logger.info("[deep-link] 已清理遗留的用户级 yoyo-code.desktop，恢复系统级条目", {
      desktopFilePath,
    });
  } catch (error) {
    logger.warn("[deep-link] 清理遗留用户级 yoyo-code.desktop 失败", { desktopFilePath, error });
  }
}
...
export function registerLinuxDeepLinkProtocol(options: RegisterLinuxDeepLinkProtocolOptions): void {
  ...
  // 用户级 yoyo-code.desktop 在 XDG
  // 解析中永远优先于系统级同名条目。rpm/deb 安装后，旧 AppImage 写入的用户级条目会把
  // /usr/share/applications/yoyo-code.desktop 持续遮蔽，快捷方式和 yoyo-code:// deep link 一直
  ...
  // 用户手写的自定义 yoyo-code.desktop（无归属标记）不受影响，保留不清理。
