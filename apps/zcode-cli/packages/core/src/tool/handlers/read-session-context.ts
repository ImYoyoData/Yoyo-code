...
export const readSessionContextToolEntry: ToolEntry = {
  capability:
    "Read bounded context from another persisted Yoyo Code session by session id without modifying state",
  metadata: {
    name: READ_SESSION_CONTEXT_TOOL_NAME,
    description:
      "Read relevant or handoff context from another persisted Yoyo Code session. Use when the user references #sess_* or asks to continue from a specific prior session.",
    modelInstructions: [
      "Use when the current task needs context from a prior Yoyo Code session mentioned by id.",
  ...
  permission: {
    ...
    reason: "ReadSessionContext only reads persisted history for a target Yoyo Code session",
    riskLevel: "low",
    sideEffectScope: "session",
    needsApproval: false,
