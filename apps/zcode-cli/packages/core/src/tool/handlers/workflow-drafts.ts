...
// 这里写一个。
//
// 落点 `<cwd>/.yoyo-code/workflow-drafts/`，与 `.yoyo-code/workflows/`（用户保存的定义）、
// `.yoyo-code/workflow-runs/`（每个 run 的编译入口）平级。目录自带一份 `.gitignore: *`，写法与
// dynamic-workflow-runtime/src/child-entry-file.ts 逐字同构（那里的注释记着裁决）：只在缺席时
// 写一次，用户改过就不再动它，项目自己的 `.gitignore` 一个字都不碰。
//
// **尽力而为**：写不进去（只读 checkout、`.yoyo-code` 是个普通文件、盘满）不让调用失败，返回
...
interface WriteWorkflowDraftInput {
  /**
   * 会话工作目录；草稿落在它的 `.yoyo-code/workflow-drafts/` 下。缺席即宿主没有工作目录概念
...
export async function writeWorkflowDraft(
  ...
  try {
    ...
    // 尽力而为：目录写不进、`.yoyo-code` 是个文件、盘满……一律当作「这次没有草稿」。调用方据此
    // 退回旧文案，工具调用本身照常完成。
    return undefined;
  }
}
