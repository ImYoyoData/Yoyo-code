...
export async function prepareCliProviderRuntimeEnv(
  ...
  const personalFilePath =
    explicitPersonal ?? join(dataBaseDir, ".yoyo-code", "v2", PERSONAL_PROVIDER_CONFIG_FILE_NAME);
  ...
  const cachePaths = resolveZCodeBuiltinCachePaths({
    environmentConfigRoot: join(dataBaseDir, ".yoyo-code", "v2"),
    platform,
    appVersion,
    zcodeEndpointOrigin,
  });
...
async function resolveBundledZCodeBuiltinProviderConfig(input: {
  ...
  if (input.sea?.isSea()) {
    ...
    return materializeZCodeBuiltinProviderConfig({
      environmentConfigRoot: join(input.dataBaseDir, ".yoyo-code", "v2"),
      content,
    });
  }
