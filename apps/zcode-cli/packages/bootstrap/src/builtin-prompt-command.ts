...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running Yoyo Code's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future Yoyo Code agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".yoyo-code", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.yoyo-code/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .yoyo-code/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future Yoyo Code agents would otherwise miss.",
