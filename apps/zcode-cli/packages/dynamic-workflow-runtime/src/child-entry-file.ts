 ...
 * `<cwd>/.yoyo-code/workflow-runs/<runId>.mjs`，spawn 时命令行只剩这条路径。
 *
 * 裁决：
 *   - 位置 `.yoyo-code/workflow-runs/`，与项目级 saved workflow 的 `.yoyo-code/workflows/` 同级不混放；
 *   - 文件**保留**不删（同一 runId 原位覆写），目录兼作每次 run 实际执行体的存档；
 *   - 目录里由本模块写一份 `.gitignore`（`*`），只在缺席时写一次，绝不碰项目自己的 `.gitignore`；
 *   - 项目目录写不进（只读 checkout、cwd 不存在、`.yoyo-code` 是个普通文件…）→ 回落到
...
export interface HarnessWarning {
  ...
  /** 本想写入的项目目录（`<cwd>/.yoyo-code/workflow-runs`）。 */
  projectDir: string;
  /** 实际回落到的目录。 */
  fallbackDir: string;
  /** 项目目录写入失败的错误文本。 */
...
export function workflowRunsDir(cwd: string): string {
  return join(cwd, ".yoyo-code", "workflow-runs");
}
